...
async def test_deriv_api_calls(mocker):
    ...
    assert (await api.accountClosure({'account_closure': 1, 'reason': 'want'})) == {'account_closure': 1,
                                                                                    'reason': 'want'}, 'accountClosure can get right result'
...
def test_parse_parse_args():
    assert deriv_api_calls.parseArgs(
    ...
    assert deriv_api_calls.parseArgs({'config': {'acc': {'type': 'string'}}, 'args': {'hello': 0}, 'method': 'acc',
